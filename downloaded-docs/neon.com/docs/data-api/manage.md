> This page location: Backend > Data API > Using the Data API > Managing the Data API
...
This page covers managing the Data API after it is enabled. To enable via the Console, see [Getting started with Data API](https://neon.com/docs/data-api/get-started). To enable programmatically, see [Enable via the Neon API](https://neon.com/docs/data-api/manage#enable). To disable from the Console, scroll to **Disable** at the bottom of the **Settings** tab.

## Manage authentication providers

You can configure which authentication provider validates JWT tokens for your Data API requests. Only one provider can be configured at a time. If you enabled the Data API with [Neon Auth](https://neon.com/docs/auth/overview), it is already set as the default provider.
...
- **Neon Auth:** `-d '{"auth_provider": "neon_auth", "add_default_grants": true}'`
  ...
  If Neon Auth is not already enabled on the branch, this automatically provisions it. The optional `add_default_grants` option grants authenticated users permissions on tables in the `public` schema, matching the default Console behavior. See [Neon Auth](https://neon.com/docs/auth/overview) to learn more.
