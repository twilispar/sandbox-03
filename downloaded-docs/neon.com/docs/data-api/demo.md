> This page location: Backend > Data API > Using the Data API > Building a note-taking app
...
This enables both the Data API and Neon Auth in one step. For detailed instructions, see [Getting started with Data API](https://neon.com/docs/data-api/get-started).
...
Neon's RLS policies use the auth.user_id() function, which extracts the user's ID from the JWT (JSON Web Token) provided by your authentication provider. In this demo, [Neon Auth](https://neon.com/docs/auth/overview) issues the JWTs, and Neon's Data API passes them to Postgres, so RLS can enforce per-user access.
