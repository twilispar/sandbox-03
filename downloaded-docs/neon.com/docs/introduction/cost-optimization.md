> This page location: Plans and billing > Cost optimization
...
Strategies to manage and reduce your Neon costs
...
To monitor your current usage, check the **Billing** page in the [Neon Console](https://console.neon.tech), which shows your charges to date for each billing metric. For programmatic access to usage data, see [Consumption metrics](https://neon.com/docs/guides/consumption-metrics).
...
- **Consider read replica costs.** [Read replicas](https://neon.com/docs/introduction/read-replicas) are billed as separate compute endpoints. If you have read replicas, they add to your total compute hours. Review whether all replicas are needed, and consider scale to zero settings for replicas used infrequently.
...
- **Manage child branch storage.** Child branches are billed for the minimum of accumulated data changes or your logical data size, capped at your actual data size. While this prevents charges from exceeding your data size, managing branches effectively still helps minimize costs:
  ...
  - [Delete](https://neon.com/docs/manage/branches#delete-a-branch) child branches when they're no longer needed
  - For production workloads, use a [root branch](https://neon.com/docs/manage/branches#root-branch) instead (root branches are billed on actual data size)

- **Implement branch lifecycle management.** Review your branches regularly and [delete](https://neon.com/docs/manage/branches#delete-a-branch) any that are no longer needed. Keeping your branch count under control reduces both storage costs and potential [extra branch charges](https://neon.com/docs/introduction/plans#extra-branches).
...
Yes. Any data-modifying operation, including deletes, generates [WAL records](https://neon.com/docs/reference/glossary#write-ahead-logging-wal) that temporarily increase your storage size until they age out of your history window. For mass deletions, `TRUNCATE TABLE` is more storage-efficient since it logs a single operation rather than a record for each deleted row.
...
- **Free plan**: The Free plan includes 0.5 GB of storage per project. If you reach this limit, database operations that would increase storage (inserts, updates, and deletes) will fail until you reduce your storage or [upgrade to a paid plan](https://neon.com/docs/introduction/manage-billing#change-your-plan).
...
- **Instant restore history:** Neon charges for point-in-time restore (PITR) storage only for branches you can point-in-time restore from: **root branches**. Child branches do not add to PITR storage charges. If you perform many data modifications on your root branch(es) with a 7-day history window, you'll accumulate 7 days of that billable history at $0.20/GB-month. See [Instant restore storage](https://neon.com/docs/introduction/cost-optimization#instant-restore-storage) for optimization strategies.
- **Unused branches:** If you created branches, performed write operations, and forgot about the branches, they could be contributing to your storage costs. Review and [delete](https://neon.com/docs/manage/branches#delete-a-branch) branches you no longer need.
- **Table bloat:** Frequent updates and deletes can cause table bloat (dead tuples), which can make your data size larger than expected. See the [VACUUM FAQ](https://neon.com/docs/introduction/cost-optimization#how-does-running-vacuum-or-vacuum-full-affect-my-storage-costs) for details.
...
- **Consider logical replication costs.** [Logical replication](https://neon.com/docs/guides/logical-replication-neon) to external destinations counts as public data transfer. If you're replicating large amounts of data, factor this into your transfer costs.

- **Reduce backup-related egress.** Frequent `pg_dump` (or similar) exports send your data over the network and count as public data transfer. To lower egress, reduce how often you run dumps and consider using [Neon's snapshots](https://neon.com/docs/guides/backup-restore) with [scheduled backups](https://neon.com/docs/guides/backup-restore#create-backup-schedules) as a backup alternative that does not transfer data off Neon.

- **Monitor with the consumption API.** Track data transfer programmatically using the [Consumption API](https://neon.com/docs/guides/consumption-metrics), or check the **Billing** page in the Console for current charges.
