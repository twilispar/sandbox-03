> This page location: Tools & Workflows > API, CLI & SDKs > SDKs > Neon API TypeScript SDK
...
Neon's [Agent Skills](https://neon.com/docs/ai/agent-skills) give AI coding assistants context about the Neon TypeScript SDK for managing resources programmatically and other Neon features. Install them for more accurate code suggestions.
...
- `name` (optional): The branch name. If not provided, defaults to the branch ID. If specified, must be unique within the project and can be up to 256 characters. Cannot be empty or consist only of whitespace. See [Branch naming requirements](https://neon.com/docs/manage/branches#branch-naming-requirements) for details.
