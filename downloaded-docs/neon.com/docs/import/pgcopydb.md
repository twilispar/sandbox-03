> This page location: Migrate to Neon > Utilities > pgcopydb
...
**Important:** `pgcopydb` uses `pg_dump` and `pg_restore` under the hood. Avoid using a [pooled connection string](https://neon.com/docs/reference/glossary#pooled-connection-string) for Neon in `PGCOPYDB_SOURCE_PGURI` or `PGCOPYDB_TARGET_PGURI`. Use an [unpooled connection string](https://neon.com/docs/reference/glossary#unpooled-connection-string) instead.
