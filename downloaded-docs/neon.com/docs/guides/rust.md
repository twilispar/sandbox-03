> This page location: Frontend & Frameworks > Languages > Rust
...
Pre-built prompt for connecting Rust applications to Neon [View prompt](https://neon.com/prompts/rust-prompt.md)
...
- Start a transaction to insert multiple book records in a single operation.
  ...
  **Info: Why use a transaction for inserting multiple rows?**

  Unlike database drivers in some other languages that offer a single high-level method for bulk inserts (like [Python's](https://neon.com/docs/guides/python#create-a-table-and-insert-data) `executemany` in `psycopg2`), the idiomatic Rust approach is to loop through the data inside a transaction.
