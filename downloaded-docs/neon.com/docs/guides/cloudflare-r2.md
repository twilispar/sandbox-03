> This page location: Tools & Workflows > Integrations (3rd party) > File & media storage > Cloudflare R2
...
3. Click **Create bucket**, provide a unique bucket name (for example, `my-neon-app-files`), and click **Create bucket**.
   ![Create R2 Bucket](https://neon.com/docs/guides/cloudflare-r2-create-bucket.png)
...
If you use [Neon's Row Level Security (RLS)](https://neon.com/blog/introducing-neon-authorize), remember to apply appropriate access policies to the `r2_files` table. This controls who can view or modify the object references stored in Neon based on your RLS rules.
...
- Use this `file_url` in your application (for example, `<img>` tags, API responses, download links) wherever you need to display or provide access to the file.
  ...
  **Note: Private buckets**
  ...
  For private R2 buckets, store only the `object_key` and generate presigned _read_ URLs on demand using a similar backend process.
