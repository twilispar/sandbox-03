> This page location: Frontend & Frameworks > ORMs > Elixir Ecto
...
Pre-built prompt for connecting Elixir applications to Neon with Ecto [View prompt](https://neon.com/prompts/elixir-ecto-prompt.md)
...
6. Enter a database name (`friends`), and select a database owner.
7. Click **Create**.
...
1. Navigate to the [Neon Console](https://console.neon.tech).
2. Select a project.
3. Select **Tables** from the sidebar.
4. Select the Branch, Database (`friends`), and the schema (`public`). You should see the `people` table along with a `schema_migration` table that was created by the migration.
