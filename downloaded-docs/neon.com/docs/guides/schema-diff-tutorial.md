> This page location: Data recovery > Schema diff > Schema diff tutorial
...
2. Verify the schema on your new branch
   ...
   ![use metacommand to inspect schema](https://neon.com/docs/guides/schema_diff_d_metacommand.png)
...
2. Verify the schema
   ...
   2. Connect to `people` using psql.
   ...
   3. View the schema for the `person` table we created earlier.
...
2. Connect to the `people` database on the `feature/address` branch with `psql`:
...
1. Click on `feature/address` to open the detailed view, then click **Schema diff**.
   ...
   ![select branches for schema diff](https://neon.com/docs/guides/schema_diff_make_selection.png)
