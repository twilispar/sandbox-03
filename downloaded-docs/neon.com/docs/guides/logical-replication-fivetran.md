> This page location: Tools & Workflows > Integrations (3rd party) > Replicate from Neon > Fivetran
...
4. Enter the connection details for your Neon database. You can find your Neon database connection details by clicking the **Connect** button on your **Project Dashboard** to open the **Connect to your database** modal.
...
5. For **Connection Method**, select **Logical replication of the WAL using the pgoutput plugin** and enter values for the **Replication Slot** and **Publication Name**. You defined these values earlier (`fivetran_pgoutput_slot` and `fivetran_pub`, respectively).

   ![Fivetran connector setup](https://neon.com/docs/guides/fivetran_connector_setup.png)
...
6. If you are using Neon's **IP Allow** feature to limit IP addresses that can connect to Neon, add Fivetran's IPs to your allowlist in Neon.
   ...
   ![Fivetran IP addresses](https://neon.com/docs/guides/fivetran_ips.png)
...
7. Click **Save & Test**. Fivetran tests and validates the connection to your database. Upon successful completion of the setup tests, you can sync your data using Fivetran.
   ...
   ![Fivetran all connections passed message](https://neon.com/docs/guides/fivetran_connection_test.png)

8. Click **Continue**.

9. On the **Select Data to Sync** page, review the connector schema and select any columns you want to block or hash.

   ![Fivetran select data to sync page](https://neon.com/docs/guides/fivetran_select_data.png)
...
11. On the **How would you like to handle changes?** page, specify how you would like to handle future schema changes. For this example, we'll select **We will allow all new schemas, tables and columns**. Choose the option that best fits your organization's requirements.
    ...
    ![Fivetran how to handle changes](https://neon.com/docs/guides/fivetran_changes.png)

12. Click **Continue**. Your data is now ready to sync.

    ![Fivetran data is ready to sync page](https://neon.com/docs/guides/fivetran_ready_to_sync.png)
