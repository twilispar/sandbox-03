> This page location: Data recovery > Backup & restore
...
You can restore from any time that falls within your project's [history window](https://neon.com/docs/introduction/history-window).
...
You can create a snapshot from a branch using the [Create snapshot](https://api-docs.neon.tech/reference/createsnapshot) endpoint. A snapshot can be created from a specific timestamp (RFC 3339 format) or LSN (for example 16/B3733C50) within the branch's [history window](https://neon.com/docs/introduction/history-window). The `timestamp` and `lsn` parameters are mutually exclusive; you can use one or the other, not both.
...
1. **Open the schedule editor**
   ...
   ![Edit schedule button on Backup & restore page](https://neon.com/docs/guides/edit_snapshot_schedule.png)
...
2. **Select a schedule frequency**
   ...
   ![Schedule frequency options dropdown](https://neon.com/docs/guides/snapshot_schedule_menu.png)
...
![Edit backup schedule modal](https://neon.com/docs/guides/edit_backup_schedule_modal.png)

**API**

To update a backup schedule via API, use the same PUT endpoint and request format as for creating a schedule. See [Create backup schedules](https://neon.com/docs/guides/backup-restore#create-backup-schedules) for the endpoint, request body parameters, and example.
...
1. Locate the snapshot you want to use and click **Restore → One-step restore**.
   ...
   ![One step restore option](https://neon.com/docs/guides/one_step_restore.png)
...
2. The **One-step restore** modal explains the operation:
   ...
   ![One step restore confirmation modal](https://neon.com/docs/guides/one_step_restore_modal.png)
...
3. Your branch is immediately restored to the snapshot state, and the `<branch_name>_old` branch is created, which you'll find on the **Branches** page in the Neon Console, as shown here:
   ![Branches page that shows the backup branch](https://neon.com/docs/guides/one_step_restore_branches_page.png)
...
1. Locate the snapshot you want to use and click **Restore → Multi-step restore**.
   ![Multi-step restore option](https://neon.com/docs/guides/multi_step_restore.png)
...
2. The **Multi-step restore** modal explains the operation:
   ...
   ![Multi-step restore confirmation modal](https://neon.com/docs/guides/multi_step_restore_modal.png)
...
3. Clicking **Restore** creates the new branch with the restored data and directs you to the **Branch overview** page where you can:
   ...
   ![Branch overview page](https://neon.com/docs/guides/branch_overview_page.png)
...
3. **Finalize the restore**
   ...
   - If the original branch was **protected**, that protection is **moved** to the branch that ends up with your restored data (the renamed branch that keeps your connection string). The previous branch is no longer protected, so your [protected branch](https://neon.com/docs/guides/protected-branches) count stays correct.
...
- **Reset from parent is unavailable on child branches for up to 24 hours after restoring a parent from a snapshot.** When you restore a branch from a snapshot, any child branches of that restored branch cannot use the [Reset from parent](https://neon.com/docs/guides/reset-from-parent) feature for up to 24 hours.
