> This page location: Tools & Workflows > Integrations (3rd party) > File & media storage > Backblaze B2
...
3. Click **Create a Bucket**. Provide a globally unique bucket name (for example, `my-neon-app-b2-files`), choose whether files should be **Private** or **Public**. For this guide, we'll use **Public** for simplicity, but **Private** is recommended for production applications where you want to control access to files.
   ![Create B2 Bucket](https://neon.com/docs/guides/backblaze-b2-create-bucket.png)
4. **Create application key:**
   ...
   - Copy the **Key ID** and **Application Key**. These will be used in your application to authenticate with B2.
     ![Create B2 Application Key](https://neon.com/docs/guides/backblaze-b2-create-app-key.png)
5. **Find S3 endpoint:**
   ...
   - Find your bucket and note the **Endpoint** URL listed (for example, `s3.us-west-000.backblazeb2.com`). You'll need this S3-compatible endpoint for the SDK configuration.
     ![B2 Bucket Endpoint](https://neon.com/docs/guides/backblaze-b2-bucket-endpoint.png)
...
![Example CORS configuration](https://neon.com/docs/guides/backblaze-b2-cors-example.png)
...
If you use [Neon's Row Level Security (RLS)](https://neon.com/blog/introducing-neon-authorize), remember to apply appropriate access policies to the `b2_files` table. This controls who can view or modify the object references stored in Neon based on your RLS rules.
