> This page location: Tools & Workflows > Integrations (3rd party) > Replicate from Neon > Databricks
> Full Neon documentation index: https://neon.com/docs/llms.txt

# Replicate data to Databricks with Lakeflow Connect

Learn how to replicate data from Neon to Databricks Lakehouse using the Lakeflow Connect PostgreSQL connector
...
- **A Neon project** with a Postgres database containing the data you want to replicate. For test data, you can run the following in the [Neon SQL Editor](https://neon.com/docs/get-started/query-with-neon-sql-editor) or an SQL client such as [psql](https://neon.com/docs/connect/query-with-psql-editor):
...
- **Network connectivity**: Neon must accept inbound connections from your Databricks workspace (or via VPN/PrivateLink, depending on setup). If you use [IP Allow](https://neon.com/docs/manage/projects#configure-ip-allow) in Neon, add the Databricks egress addresses to your allowlist. See [Subscriber access](https://neon.com/docs/guides/logical-replication-neon#subscriber-access)
...
**Important:** Enabling logical replication sets `wal_level` from `replica` to `logical` for the entire project and restarts all computes in the project, briefly disconnecting clients. Once set to `logical`, it cannot be reverted.
...
- Periodically check replication slots on Neon and [drop unused slots](https://neon.com/docs/guides/logical-replication-manage#remove-a-replication-slot) if you delete pipelines; Databricks does not drop slots automatically.
...
For adding tables, monitoring lag, and cleaning up slots, see Databricks' [Maintain PostgreSQL ingestion pipelines](https://docs.databricks.com/aws/en/ingestion/lakeflow-connect/postgresql-maintenance) documentation and [Logical replication in Neon](https://neon.com/docs/guides/logical-replication-neon).
...
- [Logical replication in Neon](https://neon.com/docs/guides/logical-replication-neon)
- [Logical replication commands](https://neon.com/docs/guides/logical-replication-manage)
