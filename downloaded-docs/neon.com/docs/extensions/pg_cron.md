> This page location: Extensions > pg_cron
...
**Warning: Key details about using pg_cron with Neon**

Please note that `pg_cron` jobs will only run when your compute is active. We therefore recommend only using `pg_cron` on computes that run 24/7 or where you have disabled [scale to zero](https://neon.com/docs/introduction/scale-to-zero).
...
**Warning: Function not supported in Neon**
...
**Note: Important: Setting Modifications in Neon**
